      ...
      <div className="container mx-auto ">
        {user ? (
          <div className="pt-32 flex flex-col items-center w-2/3 lg:w-1/2 mx-auto">
            ...
            <div className="w-full">
              <div className="flex items-center">
                <img className="rounded-full w-32 h-32" src={user.image} alt="https://via.placeholder.com/150" />
                <div className="ps-5">
                  <div className="flex justify-between items-center mb-5">
                    <h1 className="text-3xl font-bold text-center me-4 text-gray-700">@ {user.username}</h1>
                    {id != localStorage.getItem("id") && (
                      <button
                        className="bg-gray-600 text-white rounded-lg py-1 px-3 font-semibold hover:bg-gray-500"
                        onClick={() => {
            ...
            <hr className="w-full mx-auto mt-2 mb-10 border-gray-200" />
          ...
          <div className="w-full h-screen flex justify-center items-center">
            <p className="text-gray-700 text-2xl font-semibold">Loading...</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed w-screen h-screen top-0 left-0 bottom-0 bg-[rgba(27,28,24,0.34)]">
          <div className="w-[80%] xl:w-[60%] h-[70%] mt-[8%] mx-auto bg-white flex">
            ...
            <div className="flex flex-col w-full">
              {/* card header */}
              <div className="flex justify-start items-center p-2 border-b-2 border-gray-200 h-[12%]">
                <img className="w-[40px] rounded-full me-5" src={userPost.user.image} alt="" />
                <span className="font-semibold me-auto text-gray-700">@ {userPost.user.username}</span>
              </div>
                  ...
                  userPost.comments.map((comment, index) => (
                    <div className="p-2 flex items-center justify-between" key={index}>
                      ...
                      {comment.user._id == logInUser._id && (
                        ...
                        >
                          <span className="material-symbols-outlined text-red-800 text-lg font-bold">close</span>
                        </div>
                      )}
                    </div>
                  ))
              ...
              {/* comment form */}
              <div className="flex items-center h-[12%] border-t-2 border-gray-200 p-2">
                <span className="material-symbols-outlined text-gray-600">mood</span>
                <input
                ...
                <button
                  ...
                  className="px-1 pb-2 text-lg text-gray-800 font-semibold"
                >
                  post
                </button>
              </div>
          ...
          >
            <span className="material-symbols-outlined text-gray-50 text-4xl font-bold">close</span>
          </div>
        </div>
      )}
    </>
